...
export const AuthScreen: React.FC = () => {
  ...
  // Real Google OAuth Popup Handler
  const handleGoogleClick = () => {
    ...
    if (clientId && window.google?.accounts?.oauth2) {
            ...
            if (tokenResponse.access_token) {
              try {
                ...
                if (userInfo.email) {
                  ...
                  setError('Could not retrieve email from your Google account.')
                }
              } catch {
                setError('Failed to authenticate Google account with backend.')
              } finally {
                setIsGoogleLoading(false)
              }
            }
      ...
      // If Google Client ID is not yet provided in env, show instant Google login dialog
      setIsGoogleModalOpen(true)
    }
  }
